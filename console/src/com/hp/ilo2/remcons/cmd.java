    ...
    public boolean connectCmd(remcons remconsVar, String str, int i) {
        try {
            ...
            if (this.in.readByte() == 80) {
                bArr2[0] = 2;
                String rcInfo1 = remconsVar.ParentApp.getParameter("RCINFO1");
                byte[] bytes = (rcInfo1 != null) ? rcInfo1.getBytes() : new byte[0];
                if (remconsVar.ParentApp.optional_features.contains("ENCRYPT_KEY") && remconsVar.ParentApp.enc_key != null && !remconsVar.ParentApp.enc_key.isEmpty()) {
