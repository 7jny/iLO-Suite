...
public class intgapp extends JApplet implements Runnable, ActionListener, ItemListener {
    ...
    public void init() {
        ...
        System.out.println("Completed Retrieving parameters from ILO: " + (jSONRequest != null ? "OK" : "FAILED"));
        ...
        if (null == jSONRequest) {
            ...
            throw new RuntimeException("Verbindungsparameter (rc_info) konnten nicht von iLO abgerufen werden.");
        }
    }
    ...
    public void start() {
        try {
            ...
            new Thread(this, "intgapp-devices").start();
        } catch (Exception e) {
    ...
    public void updatePsMenu(int i) {
        SwingUtilities.invokeLater(() -> {
            try {
                if (this.psMenu == null) return;
                if (this.psMenu.getItemCount() < 4) {
                    this.psMenu.removeAll();
                    if (this.momPress != null) this.psMenu.add(this.momPress);
                    if (this.pressHold != null) this.psMenu.add(this.pressHold);
                    if (this.powerCycle != null) this.psMenu.add(this.powerCycle);
                    if (this.sysReset != null) this.psMenu.add(this.sysReset);
                }
                if (this.momPress != null) this.momPress.setEnabled(true);
                if (this.pressHold != null) this.pressHold.setEnabled(true);
                if (this.powerCycle != null) this.powerCycle.setEnabled(true);
                if (this.sysReset != null) this.sysReset.setEnabled(true);
                this.psMenu.revalidate();
                this.psMenu.repaint();
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void ApplyRcInfoParameters(String str) {
        ...
        if (str == null || str.trim().isEmpty()) {
            System.err.println("[intgapp] Warning: ApplyRcInfoParameters received null or empty response");
            return;
        }
        String trim = str.trim();
        System.out.println("[intgapp] Applying rc_info parameters: " + trim);
        ...
        for (String str2 : trim.split(",")) {
            ...
            if (key.compareToIgnoreCase("enc_key") == 0) {
                this.enc_key = val;
                for (int i = 0; i < this.enc_key_val.length && (i * 2 + 2) <= this.enc_key.length(); i++) {
                    try {
                        this.enc_key_val[i] = (byte) Integer.parseInt(this.enc_key.substring(i * 2, (i * 2) + 2), 16);
                    } catch (NumberFormatException e) {
                        System.out.println("Failed to Parse enc_key: " + e.getMessage());
                    }
                }
            } else if (key.compareToIgnoreCase("rc_port") == 0) {
                System.out.println("rc_port:" + val);
                this.rc_port = val;
            } else if (key.compareToIgnoreCase("vm_key") == 0) {
                this.vm_key = val;
            } else if (key.compareToIgnoreCase("vm_port") == 0) {
                System.out.println("vm_port:" + val);
                this.vm_port = val;
            } else if (key.equalsIgnoreCase("optional_features")) {
                System.out.println("optional_features:" + val);
                this.optional_features = val;
            } else if (key.compareToIgnoreCase("server_name") == 0) {
                System.out.println("server_name:" + val);
                this.server_name = val;
            } else if (key.compareToIgnoreCase("ilo_fqdn") == 0) {
                System.out.println("ilo_fqdn:" + val);
                this.ilo_fqdn = val;
            } else if (key.compareToIgnoreCase("blade") == 0) {
                try {
                    this.blade = Integer.parseInt(val);
                    System.out.println("blade:" + this.blade);
                } catch (NumberFormatException ignored) {}
            } else if (this.blade == 1 && key.compareToIgnoreCase("enclosure") == 0) {
                if (!val.equals("null")) {
                    this.in_enclosure = true;
                    System.out.println("enclosure:" + val);
                    this.enclosure = val;
                }
            } else if (this.blade == 1 && key.compareToIgnoreCase("bay") == 0) {
                try {
                    this.bay = Integer.parseInt(val);
